...
from ffr.trainer import Qwen2VLGRPOTrainer, Qwen2VLGRPOVLLMTrainerModified
...
class GRPOScriptArguments(ScriptArguments):
    ...
    use_ffr: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to use Find-Fix-Reason (FFR) framework"}
    )
    teacher_api_url: Optional[str] = field(
        default="http://localhost:8000",
        metadata={"help": "Teacher model API URL for FFR framework"}
    )
    teacher_api_nframes: Optional[int] = field(
        default=16,
        metadata={"help": "Number of frames to send to teacher API"}
    )
    patch_tax: Optional[float] = field(
        default=0.0,
        metadata={"help": "Patch tax (kappa) for FFR framework - penalty for using teacher evidence"}
    )
...
def main(script_args, training_args, model_args):
    ...
    trainer = trainer_cls(
        ...
        use_ffr=script_args.use_ffr,
        teacher_api_url=script_args.teacher_api_url,
        teacher_api_nframes=script_args.teacher_api_nframes,
        patch_tax=script_args.patch_tax,
    )
